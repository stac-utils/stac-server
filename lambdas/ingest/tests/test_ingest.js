...
const esClient = require('../../../libs/esClient')
const awsClients = require('../../../libs/aws-clients')
...
const getCollectionIds = async () => {
  const es = await esClient.connect()

  const indicesResponse = await es.cat.indices({ format: 'json' })

  return indicesResponse.body.map((c) => c.index)
}
