...
Runtime configuration reference for stac-server environment variables and collection-level parameters.
...
| **ENABLE_CONTEXT_EXTENSION** | Enable deprecated [Context Extension](https://github.com/stac-api-extensions/context) `context` object | `false` | **Deprecated**: Adds legacy `context.matched`/`returned`/`limit` object. Note: `numberMatched` and `numberReturned` are always returned at root level per STAC API spec |
...
| **REQUEST_LOGGING_FORMAT** | [Morgan](https://github.com/expressjs/morgan) HTTP request logging format | `tiny` | `combined` (Apache combined), `common` (Apache common), `dev` (colored for development), `short`, `tiny` (minimal), or custom format string |
...
The `aggregations` array in a [STAC Collection](https://github.com/radiantearth/stac-spec/blob/master/collection-spec/collection-spec.md) defines which statistical summaries and frequency distributions are available through the [Aggregation Extension](https://github.com/stac-api-extensions/aggregation).
...
3. **Context Extension**: Leave ENABLE_CONTEXT_EXTENSION disabled (deprecated extension, not needed since `numberMatched`/`numberReturned` are always returned)
