...
test(
  'POST /collections index hasing allows creation of collections with same name but different case',
  async (t) => {
    const collectionIdOne = 'TestCollection'
    const response = await t.context.api.client.post(
      'collections',
      { json: { id: collectionIdOne }, resolveBodyOnly: false, responseType: 'text' }
    )
    // ES needs a second to process the create request
    // eslint-disable-next-line no-promise-executor-return
    await new Promise((r) => setTimeout(r, 1000))
    ...
    const response2 = await t.context.api.client.post(
      'collections',
      { json: { id: collectionIdTwo }, resolveBodyOnly: false, responseType: 'text' }
    )
    // ES needs a second to process the create request
    // eslint-disable-next-line no-promise-executor-return
    await new Promise((r) => setTimeout(r, 1000))
    ...
    const response3 = await t.context.api.client.get(
      `collections/${collectionIdOne}`,
      { resolveBodyOnly: false }
    )
    ...
    const response4 = await t.context.api.client.get(
      `collections/${collectionIdTwo}`,
      { resolveBodyOnly: false }
    )
