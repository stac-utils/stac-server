...
const opensearch = require('../../lib/database')
...
app.get('/', async (req, res, next) => {
  try {
    res.json(await api.getCatalog(txnEnabled, opensearch, req.endpoint))
  } catch (error) {
    next(error)
  }
})
...
app.get('/search', async (req, res, next) => {
  try {
    ...
    res.json(await api.searchItems(null, req.query, opensearch, req.endpoint, 'GET'))
  } catch (error) {
...
app.post('/search', async (req, res, next) => {
  try {
    res.type('application/geo+json')
    res.json(await api.searchItems(null, req.body, opensearch, req.endpoint, 'POST'))
  } catch (error) {
    if (error instanceof api.ValidationError) {
      next(createError(400, error.message))
    } else {
...
app.get('/aggregate', async (req, res, next) => {
  try {
    res.json(await api.aggregate(req.query, opensearch, req.endpoint, 'GET'))
  } catch (error) {
    if (error instanceof api.ValidationError) {
      next(createError(400, error.message))
    } else {
...
app.post('/aggregate', async (req, res, next) => {
  try {
    res.json(await api.aggregate(req.body, opensearch, req.endpoint, 'POST'))
  } catch (error) {
    if (error instanceof api.ValidationError) {
      next(createError(400, error.message))
    } else {
...
app.get('/collections', async (req, res, next) => {
  try {
    res.json(await api.getCollections(opensearch, req.endpoint))
  } catch (error) {
    next(error)
  }
})
...
app.post('/collections', async (req, res, next) => {
  if (txnEnabled) {
    ...
    try {
      await api.createCollection(req.body, opensearch)
...
app.get('/collections/:collectionId', async (req, res, next) => {
  ...
  try {
    const response = await api.getCollection(collectionId, opensearch, req.endpoint)
...
app.post('/collections/:collectionId/items', async (req, res, next) => {
  if (txnEnabled) {
    ...
    if (req.body.collection && req.body.collection !== collectionId) {
      ...
      const collectionRes = await api.getCollection(collectionId, opensearch, req.endpoint)
      if (collectionRes instanceof Error) next(createError(404))
      try {
        req.body.collection = collectionId
        await api.createItem(req.body, opensearch)
...
app.put('/collections/:collectionId/items/:itemId', async (req, res, next) => {
  if (txnEnabled) {
    ...
    if (req.body.collection && req.body.collection !== collectionId) {
      ...
      const itemRes = await api.getItem(collectionId, itemId, opensearch, req.endpoint)
      if (itemRes instanceof Error) next(createError(404))
      else {
        req.body.collection = collectionId
        req.body.id = itemId
        try {
          await api.updateItem(req.body, opensearch)
          res.sendStatus(204)
        } catch (error) {
          if (error instanceof Error
                  && error.name === 'ResponseError'
      ...
      if (itemRes instanceof Error) next(createError(404))
        ...
        try {
          ...
          await api.partialUpdateItem(
            collectionId, itemId, req.body, opensearch, req.endpoint
          )
          // res.type('application/geo+json')
          // res.json(item)
          res.sendStatus(204)
...
app.delete('/collections/:collectionId/items/:itemId', async (req, res, next) => {
  if (txnEnabled) {
    const { collectionId, itemId } = req.params
    try {
      const response = await api.deleteItem(collectionId, itemId, opensearch)
