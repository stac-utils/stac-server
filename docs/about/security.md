...
1. **Enable Fine-grained Access Control**: Use OpenSearch's built-in fine-grained access control for granular permissions (see [Deployment Guide](../deployment/index.md))
...
1. **Authentication**: Consider implementing authentication for transaction endpoints (see [Deployment Guide](../deployment/index.md) for IP-based restrictions)

2. **Rate Limiting**: Configure API Gateway throttling to prevent abuse

3. **WAF**: Use AWS WAF to protect against common web exploits (note the SQL injection false positive in [Deployment Guide](../deployment/index.md))
...
- **Guides > [Deployment](../deployment/index.md)** - Deploy securely to AWS
- **Guides > [Configuration](../configuration/index.md)** - Configure security settings
- **About > [Contributing](../development/contributing.md)** - Report security issues
